        ...
        if ($source_list) {
            ...
            if (! empty($deleteEntries)) {
                ...
                try {
                    if ($this->type == 'poi') {
                        ...
                        $ec_ids = array_column($ec_ids_result, 'id');
                        if (!empty($ec_ids)) EcPoi::whereIn('id', $ec_ids)->delete();
                    ...
                    if ($this->type == 'track') {
                        ...
                        $ec_ids = array_column($ec_ids_result, 'id');
                        if (!empty($ec_ids)) EcTrack::whereIn('id', $ec_ids)->delete();
            ...
            foreach ($source_list as $id => $updated_at) {
                if (empty($osf_list) || ! array_key_exists($id, $osf_list) || Carbon::parse($osf_list[$id]) < Carbon::parse($updated_at)) {
                    ...
                    $OSF = new OutSourceImporterFeatureSentieriSardegna($id, $this->type, $this->endpoint, $this->only_related_url, $categorie_fruibilita_sentieri, $this->logChannel);
