...
- [x] Ogni messaggio di console (info/errore) del comando viene anche persistito su un canale di log dedicato (`update_pois_from_osm`, rotazione giornaliera in `storage/logs/`), sia per i run manuali sia per quello schedulato — così un run notturno lascia traccia su disco, non solo su stdout
...
- `config/logging.php` (repo principale `geohub`) — nuovo canale `update_pois_from_osm` (driver `daily`, 14 giorni di retention) per persistere l'output del comando
